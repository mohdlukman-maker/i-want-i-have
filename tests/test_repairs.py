...
class PrototypeRepairTests(unittest.TestCase):
    ...
    def test_app_is_localized_for_kuching_phone_accessories(self):
        for required in (
            "Kuching, Sarawak",
            "RM 109",
            "Kenyalang Mobile Accessories",
            "Riverfront Gadget Shop",
            "Kuching Phone Corner",
        ...
        self.assertGreaterEqual(self.html.count(">KM</div>"), 2)
        self.assertNotIn(">SM</div>", self.html)
        self.assertIn("Add from invoice", self.html)
        self.assertIn("Use an existing stock list", self.html)
        self.assertNotIn("restaurants & food stalls", self.html)

    def test_moderator_materials_define_the_kuching_phone_accessory_pilot(self):
        root = HTML_PATH.parent.parent
        script = (root / "USER_TEST_SCRIPT.md").read_text(encoding="utf-8")
        readme = (root / "README - START HERE.txt").read_text(encoding="utf-8")
        for required in (
            "Kuching phone-accessory pilot",
            "one 3–5 km area in Kuching",
            "5–8 participating shops",
            "10–15 potential buyers",
            "phone-accessory shelf",
        ):
            self.assertIn(required, script)
        self.assertIn("Kuching, Sarawak and Malaysian-ringgit values", readme)
        self.assertNotIn("BGC", readme)
        self.assertNotIn("Taguig", readme)
